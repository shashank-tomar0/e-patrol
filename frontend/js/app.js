...
function openModal(record) {
    ...
    if (record.severity === 'critical') {
        ...
        protocol = 'Deploy patrol drone to location, broadcast pre-recorded warning over PA, and flag for dispatcher review.';
    }
    document.getElementById('modal-actions').innerText = protocol;

    // Hook up export dossier download
    document.getElementById('btn-export-dossier').onclick = () => exportDossier(record);
}
...
// Export official police case report file
function exportDossier(record) {
    const timeString = new Date(record.timestamp * 1000).toISOString().replace('T', ' ').substring(0, 19) + ' UTC';
    let protocol = 'Monitor scene telemetry bounds.';
    if (record.severity === 'critical') {
        protocol = 'Dispatch station police squad immediately, alert central line dispatcher to hold incoming trains, and activate visual audio alarm guidance.';
    ...
    const dossierContent = `================================================================================
                    E-PATROL SECURITY DIVISION - INCIDENT DOSSIER               
================================================================================
CASE FILE REF : E-PATROL-2026-${record.id.toUpperCase()}
TIME OF EVENT : ${timeString}
LOCATION      : ${record.location}
ALERT TYPE    : ${record.type.toUpperCase()}
SEVERITY LEVEL: ${record.severity.toUpperCase()}
THREAT INDEX  : ${record.threat_level.toUpperCase()}
SYSTEM STATE  : ARCHIVED & INDEXED IN VECTOR DATABASE

--------------------------------------------------------------------------------
[1] COGNITIVE AGENT SUMMARY & REASONING
--------------------------------------------------------------------------------
${record.summary}

--------------------------------------------------------------------------------
[2] KINEMATIC & VISUAL EVIDENCE ANALYSIS
--------------------------------------------------------------------------------
${record.visual_analysis}
- Average Crowd Density: ${record.density.toFixed(1)}%
- Median Crowd Velocity: ${record.speed.toFixed(2)} m/s
- Tracked Human Entities: ${record.skeletons ? record.skeletons.length : 0}

--------------------------------------------------------------------------------
[3] RESPONSE & MITIGATION PROTOCOLS (AUTOMATICALLY DISPATCHED)
--------------------------------------------------------------------------------
${protocol}

--------------------------------------------------------------------------------
[4] VERIFIER & AUDIT TRAIL
--------------------------------------------------------------------------------
- Telemetry Intake Status: NOMINAL
- Multi-Agent Verification: PASSED
- Spatial Grid Bounds Check: OK
- Operator Status: ACKNOWLEDGED & EXPORTED

================================================================================
               Generated by E-patrol AI Behavioral Reasoning Engine            
================================================================================`;
    ...
    link.href = url;
    link.download = `incident_dossier_${record.id}.txt`;
